...
<link href="bootbox.min.css" rel="stylesheet">
	...
	<div class="btn-group" style="margin-left:2.5%;">
		  <label class="btn btn-warning active">
		    <a href="#" style="text-decoration:none ;out-line: none;color: #fff"> 人物红利榜 </a>
		  </label>
		  		  <label class="btn btn-warning">
		    <a href="userdeed.php" style="text-decoration:none ;out-line: none;color: #fff"> 人物功德榜 </a>
		  </label>
		  <label class="btn btn-warning ">
		...
		<table class="table table-bordered table-responsive" style="width: 95%;margin: 0 auto;margin-bottom:50px; color: orange;background-color: #ececec;">
			<thead >
		    	<tr>
					...
					<th style="background-color: orange;"><font color="blue">红利</font></th>
		      		<th style="background-color: orange;"><font color="blue">修为</font></th>
					<th style="background-color: orange;"><font color="blue">幻币</font></th>
		    	</tr>
		  	</thead>
		  	<tbody>
			<?php
			...
			$flag = 1;
			if($flag == 0){
				$sql = "select name,floor(level) as level,metempsychosis as chenghao,look,degree_lev,money,floor((metempsychosis%1000)/10) as toutai,floor(additional_point/100000) as hongli from yx_user WHERE
account_id BETWEEN  '1' AND '9999' order by degree_lev desc limit 0,100";
			}else{
				$sql = "select name,floor(level) as level,metempsychosis as chenghao,look,degree_lev,money,floor((metempsychosis%1000)/10) as toutai,floor(additional_point/100000) as hongli from yx_user WHERE
account_id BETWEEN  '1' AND '9999' order by hongli desc, degree_lev desc limit 0,100";
			}
			$cx = $dbh->query($sql);
			$result = $cx->fetchAll();			
			$head = "";
