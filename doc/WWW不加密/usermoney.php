...
<link href="bootbox.min.css" rel="stylesheet">
	...
	<div class="btn-group" style="margin-left:2.5%;">
		  ...
		  <label class="btn btn-warning active">
		    <a href="#" style="text-decoration:none ;out-line: none;color: #fff"> 人物幻币榜 </a>
		  </label>
		  <label class="btn btn-warning">
		...
		<table class="table table-bordered table-responsive" style="width: 95%;margin: 0 auto;margin-bottom:50px; color: orange;background-color: #ececec;">
			...
			$flag = 1;
			if($flag == 0){
				$sql = "select name,floor(level) as level,metempsychosis as chenghao,look,degree_lev,(money + money_saved) as mmm,floor((metempsychosis%1000)/10) as toutai,floor(additional_point/100000) as hongli from yx_user WHERE
account_id BETWEEN  '1' AND '9999' order by money desc limit 0,100";
			}else{
				$sql = "select name,floor(level) as level,metempsychosis as chenghao,look,degree_lev,(money + money_saved) as mmm,floor((metempsychosis%1000)/10) as toutai,floor(additional_point/100000) as hongli from yx_user WHERE
account_id BETWEEN  '1' AND '9999' order by mmm desc limit 0,100";
			}
			$cx = $dbh->query($sql);
			$result = $cx->fetchAll();
			$dbh = null;
