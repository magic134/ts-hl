...
@ccclass('RankPet75')
export class RankPet75 extends Component {
    private _view: fgui.GComponent = null;

    private datas: PetRankBean[];

    async start() {
        this._view = fgui.UIPackage.createObject("App", "RankPet75").asCom;
    ...
    private renderListItem(index: number, button: fgui.GButton) {
        ...
        button.getChild("grow_rate").text = Number(data.grow_point).toFixed(2);//data.level > 1 ? (data.grow_point / (data.level - 1)).toFixed(2) : data.grow_rate;
